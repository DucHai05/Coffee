...
const PaymentPage = () => {
    ...
    const {
        ...
        cart: cartTuState,
        maCaOpen: maCaOpenTuState,
        maCa: maCaTuState,
        totalAmount: totalTuState,
        subTotal: subTotalTuState,
        autoDiscountVal: autoDiscountValTuState,
        manualDiscountVal: manualDiscountValTuState,
        manualDiscount,
        autoDiscount,
        nameTable
    } = location.state || {};
    ...
    const [maCaOpen, setMaCaOpen] = useState(maCaOpenTuState || maCaTuState || '');
   ...
   const handleConfirmPayment = async (method) => {
            ...
            const activePromo = manualDiscount || autoDiscount;
            let finalMaKM = manualDiscount?.maKhuyenMai || autoDiscount?.maKhuyenMai || null;

            // 2. CHECK NHẸ: Nếu không đủ điều kiện thì "HỦY" phần giảm giá
            if (activePromo && activePromo.configs && activePromo.configs.length > 0) {
                const checkResult = CartHelpers.validatePromotion(activePromo, finalSubTotal, currentCart);
    ...
    return (
        <div className="payment-page-wrapper">
            <ReceiptModal 
                ...
                autoDiscountVal={autoDiscountValTuState || 0} 
                manualDiscount={manualDiscount}
                manualDiscountVal={manualDiscountValTuState || 0}
